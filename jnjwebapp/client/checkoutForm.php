...
<body>
        ...
        <div class="form-section" id="form-section">
            <form id="order-form">
                ...
                <div class="delivery-form">
                    ...
                    <select class="location-select" id="country" name="Country" required >
                ...
                <!-- Hidden Fields for Shipping Fee and Total -->
                <input  type="hidden" id="shipping-fee" name="ShippingFee">
                <input type="hidden"  id="grand-total" name="GrandTotal">
    ...
    var confirmation = confirm('Are you sure you want to cancel the process? This will remove the previously made order and its session.');
    if (confirmation) {
        try {
            const response = await fetch('cancelOrder.php');
            if (response.ok) {
...
// Event listener for beforeunload event (page close/navigate)
window.addEventListener('beforeunload', function(e) {
    // Cancel the event to show the confirmation dialog
    e.preventDefault();
    // Chrome requires a non-empty string to be returned
    e.returnValue = 'Are you sure you want to leave this page? Your order may not be saved.';
});

// Event listener for unload event (page close/navigate)
window.addEventListener('unload', function() {
    // Ensure the event listeners are removed to prevent accumulation
    window.removeEventListener('beforeunload', beforeUnloadHandler);
    window.removeEventListener('unload', unloadHandler);
});

// Define event listener functions separately for better management
...
function unloadHandler() {
    // Call cancelProcess() when the page is unloaded
    cancelProcess();
}
...
window.addEventListener('unload', unloadHandler);
        ...
        // Get userID and cartID from URL parameters
    ...
    // Append userID and cartID to form data
    formData.append('userID', userID);
    formData.append('cartID', cartID);

    // Send the form data to the server using AJAX
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'saveUserOrder.php', true);
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            if (xhr.response.success) {
                var orderID = xhr.response.orderID; // Extract the orderID from the server response
                alert(xhr.response.message);
                // Redirect the user to payment.php with the orderID appended to the URL
    ...
    // Update hidden input fields with shipping fee and grand total
    $('#shipping-fee').val(shippingFee);
    $('#grand-total').val(grandTotal);

    // You can choose to update totals in the database here, or you can wait until the form is submitted
}
    </script>
</body>
</html>